...
Definition encode_offer_state (s : offer_state) : u64 :=
  match s with
  | Idle => W64 0
  | Offer => W64 1
  | Accept => W64 2
...
Definition free_chan  {H: IntoVal V} {t} {H': IntoValTyped V t} (ch: loc) (s: chan_state.t V): iProp Σ := 
  (*"value" ∷ ch ↦s[(channel.Channel.ty t) :: "v"] (value s) ∗ *)
...
Definition own_chan  {H: IntoVal V} {t} {H': IntoValTyped V t} (ch: loc) (s: chan_state.t V) (γ: chan_names): iProp Σ := 
  "chan_ghost_frag" ∷ ghost_var γ.(state_name) (1/2)%Qp s.

Definition dead_chan  {H: IntoVal V} {t} {H': IntoValTyped V t} (ch: loc) : iProp Σ := 
  "state" ∷ ch ↦s[(channel.Channel.ty t) :: "state"]□ (encode_offer_state Closed).

   (* Extended offer state resources with ownership tracking *)
Definition offer_ghost_state  {H: IntoVal V} (γ: chan_names) (st: offer_state) (val_opt: option V) : iProp Σ :=
  match st with
  | Idle => (* idle *)
      (⌜val_opt = None⌝ ∗
      offer_idle γ.(offer_name))
  | Offer => offer_out γ.(offer_name)
  | Accept => offer_out γ.(offer_name)
  | Closed =>  ⌜val_opt = None⌝
  end.
