(* autogenerated from github.com/goose-lang/goose/model/channel_v2 *)
From New.golang Require Import defn.
Require Export New.code.github_com.goose_lang.primitive.
Require Export New.code.sync.

Definition channel : go_string := "github.com/goose-lang/goose/model/channel_v2".
...
Definition Channel__Send : val :=
  rec: "Channel__Send" "c" "T" "val" :=
    ...
    let: "$a1" := (!["T"] "val") in
    ((func_call #channel_v2.channel #"NewSendCase"%go) "T") "$a0" "$a1") in
    do:  ("sendCase" <-[#ptrT] "$r0");;;
    do:  (let: "$a0" := (![#ptrT] "sendCase") in
    let: "$a1" := #true in
    ((func_call #channel_v2.channel #"Select1"%go) "T") "$a0" "$a1");;;
    return: (#());;;
    return: #()).
...
Definition Channel__Receive : val :=
  rec: "Channel__Receive" "c" "T" <> :=
    ...
    let: "$r0" := (let: "$a0" := (![#ptrT] "c") in
    ((func_call #channel_v2.channel #"NewRecvCase"%go) "T") "$a0") in
    do:  ("recvCase" <-[#ptrT] "$r0");;;
...
Definition Channel__Close : val :=
  rec: "Channel__Close" "c" "T" <> :=
    ...
    (for: (λ: <>, (~ (![#boolT] "done"))); (λ: <>, Skip) := λ: <>,
      ...
      let: "$r0" := ((method_call #channel_v2.channel #"Channel'ptr" #"TryClose" (![#ptrT] "c") "T") #()) in
      do:  ("done" <-[#boolT] "$r0");;;
...
Definition Channel__BufferedTryReceive : val :=
  rec: "Channel__BufferedTryReceive" "c" "T" <> :=
    ...
    let: (("$ret0", "$ret1"), "$ret2") := ((method_call #channel_v2.channel #"Channel'ptr" #"BufferedTryReceiveLocked" (![#ptrT] "c") "T") #()) in
    let: "$r0" := "$ret0" in
    let: "$r1" := "$ret1" in
    let: "$r2" := "$ret2" in
...
Definition Channel__TryReceive : val :=
  rec: "Channel__TryReceive" "c" "T" "blocking" :=
    ...
    then
      let: (("$ret0", "$ret1"), "$ret2") := (((method_call #channel_v2.channel #"Channel'ptr" #"BufferedTryReceive" (![#ptrT] "c") "T") #())) in
      return: ("$ret0", "$ret1", "$ret2")
    else
      let: (("$ret0", "$ret1"), "$ret2") := ((let: "$a0" := (![#boolT] "blocking") in
      (method_call #channel_v2.channel #"Channel'ptr" #"UnbufferedTryReceive" (![#ptrT] "c") "T") "$a0")) in
      return: ("$ret0", "$ret1", "$ret2"))).
...
Definition Channel__SenderCompleteOrOffer : val :=
  rec: "Channel__SenderCompleteOrOffer" "c" "T" "val" "blocking" :=
    exception_do (let: "c" := (mem.alloc "c") in
    let: "blocking" := (mem.alloc "blocking") in
    let: "val" := (mem.alloc "val") in
    (if: (![#OfferState] (struct.field_ref (Channel "T") #"state"%go (![#ptrT] "c"))) = closed
...
Definition Channel__TrySend : val :=
  rec: "Channel__TrySend" "c" "T" "val" "blocking" :=
    ...
    then
      ...
      (method_call #channel_v2.channel #"Channel'ptr" #"BufferedTrySend" (![#ptrT] "c") "T") "$a0") in
    ...
    (method_call #channel_v2.channel #"Channel'ptr" #"SenderCompleteOrOffer" (![#ptrT] "c") "T") "$a0" "$a1") in
    ...
    then
      ...
      let: "$r0" := ((method_call #channel_v2.channel #"Channel'ptr" #"SenderCheckOfferResult" (![#ptrT] "c") "T") #()) in
  ...
  rec: "TrySelect" "T" "select_case" "blocking" :=
    ...
    then
      return: (let: "$a0" := (!["T"] (struct.field_ref (SelectCase "T") #"Value"%go (![#ptrT] "select_case"))) in
       let: "$a1" := (![#boolT] "blocking") in
       (method_call #channel_v2.channel #"Channel'ptr" #"TrySend" (![#ptrT] "channel") "T") "$a0" "$a1")
    else do:  #());;;
    ...
    then
      ...
      let: "selected" := (mem.alloc (type.zero_val #boolT)) in
      let: (("$ret0", "$ret1"), "$ret2") := (let: "$a0" := (![#boolT] "blocking") in
      (method_call #channel_v2.channel #"Channel'ptr" #"TryReceive" (![#ptrT] "channel") "T") "$a0") in
      let: "$r0" := "$ret0" in
      let: "$r1" := "$ret1" in
      let: "$r2" := "$ret2" in
      do:  ("selected" <-[#boolT] "$r0");;;
...
Definition Select1 : val :=
  rec: "Select1" "T1" "case1" "blocking" :=
    ...
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      ...
      let: "$a1" := (![#boolT] "blocking") in
      ((func_call #channel_v2.channel #"TrySelect"%go) "T1") "$a0" "$a1") in
      do:  ("selected" <-[#boolT] "$r0");;;
  ...
  rec: "TrySelectCase2" "T1" "T2" "index" "case1" "case2" "blocking" :=
    ...
    then
      return: (let: "$a0" := (![#ptrT] "case1") in
       let: "$a1" := (![#boolT] "blocking") in
       ((func_call #channel_v2.channel #"TrySelect"%go) "T1") "$a0" "$a1")
    else do:  #());;;
    (if: (![#uint64T] "index") = #(W64 1)
    then
      return: (let: "$a0" := (![#ptrT] "case2") in
       let: "$a1" := (![#boolT] "blocking") in
       ((func_call #channel_v2.channel #"TrySelect"%go) "T2") "$a0" "$a1")
    else do:  #());;;
...
Definition Select2 : val :=
  rec: "Select2" "T1" "T2" "case1" "case2" "blocking" :=
    ...
    ((func_call #channel_v2.channel #"TrySelectCase2"%go) "T1" "T2") "$a0" "$a1" "$a2" "$a3"
  ...
  rec: "TrySelectCase3" "T1" "T2" "T3" "index" "case1" "case2" "case3" "blocking" :=
    ...
    then
      return: (let: "$a0" := (![#ptrT] "case3") in
       let: "$a1" := (![#boolT] "blocking") in
       ((func_call #channel_v2.channel #"TrySelect"%go) "T3") "$a0" "$a1")
    else do:  #());;;
...
Definition Select3 : val :=
  rec: "Select3" "T1" "T2" "T3" "case1" "case2" "case3" "blocking" :=
    ...
    ((func_call #channel_v2.channel #"TrySelectCase3"%go) "T1" "T2" "T3") "$a0" "$a1" "$a2" "$a3" "$a4"
    then return: (![#uint64T] "i")
    else do:  #());;;
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      (if: let: "$a0" := (![#ptrT] "case1") in
  ...
  rec: "TrySelectCase4" "T1" "T2" "T3" "T4" "index" "case1" "case2" "case3" "case4" "blocking" :=
    ...
    then
      return: (let: "$a0" := (![#ptrT] "case4") in
       let: "$a1" := (![#boolT] "blocking") in
       ((func_call #channel_v2.channel #"TrySelect"%go) "T4") "$a0" "$a1")
    else do:  #());;;
...
Definition Select4 : val :=
  rec: "Select4" "T1" "T2" "T3" "T4" "case1" "case2" "case3" "case4" "blocking" :=
    ...
    ((func_call #channel_v2.channel #"TrySelectCase4"%go) "T1" "T2" "T3" "T4") "$a0" "$a1" "$a2" "$a3" "$a4" "$a5"
    then return: (![#uint64T] "i")
    else do:  #());;;
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      (if: let: "$a0" := (![#ptrT] "case1") in
  ...
  rec: "TrySelectCase5" "T1" "T2" "T3" "T4" "T5" "index" "case1" "case2" "case3" "case4" "case5" "blocking" :=
    ...
    then
      return: (let: "$a0" := (![#ptrT] "case5") in
       let: "$a1" := (![#boolT] "blocking") in
       ((func_call #channel_v2.channel #"TrySelect"%go) "T5") "$a0" "$a1")
    else do:  #());;;
...
Definition Select5 : val :=
  rec: "Select5" "T1" "T2" "T3" "T4" "T5" "case1" "case2" "case3" "case4" "case5" "blocking" :=
    ...
    ((func_call #channel_v2.channel #"TrySelectCase5"%go) "T1" "T2" "T3" "T4" "T5") "$a0" "$a1" "$a2" "$a3" "$a4" "$a5" "$a6"
    then return: (![#uint64T] "i")
    else do:  #());;;
    (for: (λ: <>, #true); (λ: <>, Skip) := λ: <>,
      (if: let: "$a0" := (![#ptrT] "case1") in
...
#[global] Instance info' : PkgInfo channel_v2.channel :=
  {|
    pkg_vars := vars';
...
Definition initialize' : val :=
  rec: "initialize'" <> :=
    globals.package_init channel_v2.channel (λ: <>,
